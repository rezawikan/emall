$(document).ready(function() {
	 ...
	 function selectDataBank(value){
        ...
        // console.log(dataID);
	 	...
	 	$.ajax({
	 		...
	 		type : 'POST',
	 		data : {sellerID : dataID ,seller_bankID : value},
	 		success : function(result){
	 			console.log(result);
    ...
    var a = Ladda.create( document.querySelector( 'button[name=btn-add] ') );
  ...
  $('#form-data-add').submit(function(e) {
    ...
    var fv = $form.data('formValidation');
		...
		$.ajax({
    	url: 'function/withdrawal/requestWD.php',
    	type: 'POST',
    	data: $form.serialize(),
    	success: function(result){
				var resultObj = JSON.parse(result);
  ...
  function loadWithdrawal(){
    ...
    $.ajax({
    	url: 'function/withdrawal/getDataWithdrawal.php',
    	type: 'POST',
    	data: {sellerID : dataID},
    	success: function(result){
    		console.log(result);
    		...
    		if(resultObj.empty){
    			...
    			$.each(resultObj, function(index, val) {
    				 ...
    				 number++;
    				 var newRow = $("<tr>");
    				 newRow.html("<td>"+number+"</td><td>"+number_format(val.amount)+"</td><td>"+val.bankName+"</td><td>"+val.accountNumber+"</td><td>"+val.statusWithdrawal+"</td><td>"+val.lastUpdate+"</td>");
